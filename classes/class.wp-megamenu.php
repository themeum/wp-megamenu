		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $item->type === 'wpmm_row' || $item->type === 'wpmm_col' ) {
				...
				if ( is_array( $wpmm_item_settings ) ) {
					...
					if ( $args->has_children && empty( $wpmm_item_settings['options']['hide_arrow'] ) && ! empty(
						$theme_options_array['dropdown_arrow_down']
					) ) {
						$item_output .= ' <b class="fa ' . $theme_options_array['dropdown_arrow_down'] . '"></b> ';
					}
				} else { // Sett a
...
function overrite_functions_wp_megamenu( $args ) {
	...
	if ( version_compare( $wpmm_db_version, '1.1.1', '>' ) ) {
		...
		if ( count( $auto_intergration_menu ) ) {
			...
			$item_wrap = '<div class="wpmm-fullwidth-wrap"></div><div class="wpmm-nav-wrap wpmm-main-wrap-' . $args['theme_location'] . '">' . $wpmm_on_mobile . ' ' . $brand_logo_wrap . '<ul id="%1$s" ' . apply_filters( 'wpmm_custom_attributes', '', $theme_id ) . ' class="%2$s">%3$s</ul></div>';
