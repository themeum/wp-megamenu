...
if ( ! function_exists( 'include_view' ) ) {
	/**
	 * Template include inside wp-megament
	 *
	 * @param string $view_file for file path inside views.
	 *
	 * @return void
	 */
	function include_view( $view_file ) {
		include WPMM_DIR . $view_file;
	}
}
...
if ( ! function_exists( 'get_wpmm_option' ) ) {
	/**
	 * This function is to get wpmm_option by option field name.
	 *
	 * @param string $slug is the field name of menu option.
	 *
	 * @return string
	 */
	function get_wpmm_option( $slug ) {
...
if ( ! function_exists( 'wpmm_unit_to_int' ) ) {
	/**
	 * This is to remove "px" from unite value.
	 *
	 * @param string $unit_value is the css value of any amount.
	 *
	 * @return int
	 */
...
if ( ! function_exists( 'print_row' ) ) {
	 ...
	 *
	 * @param array|object $data for pre wrapped view.
	 *
	 * @return void
	 */
...
if ( ! function_exists( 'wpmm_item_panel_columns' ) ) {
	function wpmm_item_panel_columns( $menu_item_id = 0 ) {
		...
		for ( $i = 1; $i < 10; $i++ ) {
			$output .= '<option value="' . $i . '" ' . selected( $i, $saved_column, false ) . ' >' . $i . ' ' . __(
				'Columns',
				'wp-megamenu'
			) . '</option>';
		}
...
if ( ! function_exists( 'wpmm_is_serialized' ) ) {
	/**
	 * Sanitize string for wpmm
	 *
	 * @param string $value is to sanitize.
	 * @param null   $result is a boolean.
	 *
	 * @return bool
	 */
...
if ( ! function_exists( 'array_keys_exist' ) ) {
	/**
	 * To find if array key available
	 *
	 * @param string $keys is search element.
	 * @param array  $array is the target array.
	 *
	 * @return bool
	 */
